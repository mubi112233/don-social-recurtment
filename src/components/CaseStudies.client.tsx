...
export function CaseStudiesClient({ lang }: { lang: string }) {
  ...
  useEffect(() => {
    fetchCaseStudies(lang).then((data) => {
      if (data?.caseStudies) {
        setStudies(
          data.caseStudies
            .map((cs: any) => ({ ...cs, id: cs.caseStudyId ?? cs._id ?? cs.id }))
            .filter((cs: any) => cs.id != null)
  ...
  if (loading) {
    return (
      <section id="case-studies" className={`relative ${SPACING.section} bg-background`}>
        <div className={`container mx-auto ${SPACING.container} flex justify-center py-20`}>
