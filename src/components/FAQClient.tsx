...
import { fetchApiDataClient, API_ENDPOINTS, normalizeLanguage, type FAQItem } from "@/lib/api";
import { getCopy } from "@/lib/copy";

export function FAQClient({ lang }: { lang: string }) {
  ...
  useEffect(() => {
    fetchApiDataClient<{ faqs: FAQItem[] }>(API_ENDPOINTS.FAQ, normalizeLanguage(lang))
      .then((data) => {
        if (data?.faqs) {
  ...
  if (loading) {
    return (
      <section id="faq" className="relative py-8 bg-background">
  ...
  return (
    <section id="faq" className="relative py-4 sm:py-6 md:py-8 lg:py-10 z-40 overflow-hidden bg-background text-foreground">
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 xl:px-12 relative z-10">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <Accordion type="single" collapsible value={openItem} onValueChange={setOpenItem} className="space-y-3 sm:space-y-4">
              {faqs.map((faq, index) => (
                <motion.div
          ...
          >
            <div className="p-5 sm:p-6 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent backdrop-blur-sm border-2 border-primary/20 rounded-xl sm:rounded-2xl group hover:border-primary/40 transition-all duration-300">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-base sm:text-lg font-bold text-foreground mb-1.5">{copy.qualityCardTitle}</h3>
            ...
            <div className="p-5 sm:p-6 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent backdrop-blur-sm border-2 border-primary/20 rounded-xl sm:rounded-2xl group hover:border-primary/40 transition-all duration-300">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-base sm:text-lg font-bold text-foreground mb-1.5">{copy.toolsCardTitle}</h3>
          ...
          >
            <p className="text-sm sm:text-base md:text-lg font-semibold text-foreground pr-4">{copy.stillHaveQuestionsTitle}</p>
            <p className="text-sm sm:text-base text-muted-foreground mb-4 sm:mb-5">{copy.stillHaveQuestionsText}</p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <a href="#contact" className="inline-flex items-center justify-center px-6 py-3 bg-primary text-primary-foreground font-semibold rounded-xl hover:bg-primary/90 transition-all duration-300 hover:scale-105">
                {copy.contactSupport}
              </a>
              <a href="#pricing" className="inline-flex items-center justify-center px-6 py-3 bg-transparent border-2 border-primary/30 text-foreground font-semibold rounded-xl hover:border-primary hover:bg-primary/10 transition-all duration-300">
                {copy.viewPricing}
              </a>
            </div>
          </motion.div>
